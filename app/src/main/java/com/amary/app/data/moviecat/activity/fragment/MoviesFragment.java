...
public class MoviesFragment extends BaseFragment implements MovieListView, SearchView.OnQueryTextListener {
    ...
    public boolean onQueryTextChange(String newText) {
        ...
        ArrayList<ResultMovie> filMovies = new ArrayList<>();
        for (ResultMovie resultMovie : itemMovie){
            String title = resultMovie.getTitle().toLowerCase();
            if (title.contains(newText)){
                filMovies.add(resultMovie);
            }else {
                tvErrorGet.setVisibility(View.VISIBLE);
            }
        }

        movieListAdapter.setFilter(filMovies);
        movieListAdapter.setOnItemClickListener((view, position) -> {
            Intent intent = new Intent(getActivity(), DetailMovieActivity.class);
            intent.putExtra(DetailMovieActivity.EXTRA_MOVIE, filMovies.get(position));
            startActivity(intent);
        });
        return true;
    }
